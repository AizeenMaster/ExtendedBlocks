...
use pocketmine\block\{Block, CobblestoneWall, Fence, FenceGate, Trapdoor};
...
final class CustomBlock extends Block
{
	...
	public function __construct(
		...
		$this->toolHarvestLevel = min(6, max(0, $toolHarvestLevel));
		...
		switch ($this->type) {
			case 'wall':
				$wall = new class extends CobblestoneWall {
					...
					{
						return ($block instanceof Placeholder && ($custom = $block->getBlock()) instanceof CustomBlock && $custom->getType() == 'wall') || parent::canConnect($block);
					}
				};
				$wall->position($this);
			...
			case 'fence':
				$fence = new class extends Fence {
					public function canConnect(Block $block): bool
					{
						return ($block instanceof Placeholder && ($custom = $block->getBlock()) instanceof CustomBlock && $custom->getType() == 'fence') || parent::canConnect($block);
					}
				};
				$fence->position($this);
