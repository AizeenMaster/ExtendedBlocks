...
class ItemFactory extends PMFactory
{
	...
	public static function init(): void
	{
		...
		if ($list instanceof \SplFixedArray && $list->getSize() < 65536) {
			$list->setSize(65536);
		}
	}
	...
	public static function get(int $id, int $meta = 0, int $count = 1, $tags = null): Item
	{
		...
		$list = self::getList();
		$listed = null;
		if ($id >= 0 && $id < ($list instanceof \SplFixedArray ? $list->getSize() : count($list))) {
			$listed = $list[$id] ?? null;
		}
		...
		if ($listed !== null) {
			...
			$item = new ItemBlock($id < 0 ? 255 - $id : $id, $meta, $id);
		} else {
	...
	public static function addCreativeItem(Item $item): void
	{
		...
		$path = RESOURCE_PATH . "vanilla" . DIRECTORY_SEPARATOR . "creativeitems.json";
		if (!file_exists($path)) return;

		$creativeItems = json_decode(file_get_contents($path), true);
		if (!is_array($creativeItems)) return;
